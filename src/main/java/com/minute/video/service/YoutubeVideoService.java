...
//    }
    public void saveYoutubeVideos(List<Map<String, Object>> videoList, String region) {
        for (Map<String, Object> videoMap : videoList) {
            ...
            if (!(idObj instanceof Map idMap) || !(snippetObj instanceof Map snippet)) {
                System.out.println("id 또는 snippet이 없는 데이터 건너뜀");
                continue;
            }

            String videoId = (String) idMap.get("videoId");
            if (videoId == null || videoId.isBlank()) {
                System.out.println("videoId 없음, 건너뜀");
                continue;
            }
            ...
            // 썸네일 url 파싱
            String thumbnailUrl = "";
            if (snippet.get("thumbnails") instanceof Map thumbnails &&
                    thumbnails.get("default") instanceof Map defaultThumb &&
                    defaultThumb.get("url") instanceof String url) {
                thumbnailUrl = url;
            }
            ...
            System.out.println("저장 시도: " + videoId);
            repo.save(entity);
        }
    }
}
